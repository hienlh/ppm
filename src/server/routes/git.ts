...
function assertProject(projectPath: string): string {
  const abs = resolve(projectPath);
  const projects = configService.get("projects");
  const allowed = projects.some((p) => abs === p.path || abs.startsWith(p.path + "/"));
  if (!allowed) throw new Error(`Path not within a registered project: ${abs}`);
  return abs;
}
...
export function createGitRoutes() {
  ...
  app.get("/status/:project", async (c) => {
    try {
      ...
      const path = assertProject(project);
  ...
  app.post("/stage", async (c) => {
    try {
      ...
      const path = assertProject(body.project);
