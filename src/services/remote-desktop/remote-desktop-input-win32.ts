/**
 * Windows input backend: Bun FFI straight into `user32.dll` `SendInput` — no separate helper
 * process (drops the Rust-binary build step + unsigned-exe/SmartScreen risk the phase plan
 * flagged for a bundled injector; phase 02's SYSTEM-session helper is a different
...
async function injectWheel(deltaY: number): Promise<void> {
...
async function injectKey(code: string, down: boolean): Promise<boolean> {
