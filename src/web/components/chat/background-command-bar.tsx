    ...
    <div className="shrink-0 border-b border-border bg-surface-elevated/60 px-2 py-1.5 space-y-1">
      {active.map((shell) => (
        <div key={shell.shellId} className="flex items-center gap-2 text-xs">
          ...
          {shell.status === "stopping" && <span className="text-[11px] text-amber-500 shrink-0">stopping…</span>}
          <button
            type="button"
          ...
          <button
            ...
            className="flex items-center justify-center size-8 rounded hover:bg-surface text-text-secondary hover:text-red-400 transition-colors disabled:opacity-40"
