...
describe("migratePortsToTunnels", () => {
  ...
  it("is idempotent on a blob with no legacy ports tab", () => {
    const clean = makeLayout({
      panels: { p1: { id: "p1", tabs: [tab("tunnels", "tunnels")], activeTabId: "tunnels", tabHistory: ["tunnels"] } },
    });
    const out = migratePortsToTunnels(clean);
    expect(out.panels.p1!.tabs).toHaveLength(1);
    expect(out.panels.p1!.tabs[0]!.type).toBe("tunnels");
    expect(out.panels.p1!.activeTabId).toBe("tunnels");
  });
