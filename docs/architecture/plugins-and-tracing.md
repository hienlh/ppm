...
Two files this cites are still in review rather than on `main`: `src/services/lsp/server-registry.ts`
arrives with the editor's language servers (#23) and `src/services/event-loop-lag.ts` with the
event-loop work (#26). Everything else named here exists today, except where the text says it does
not.
