...
- **A missing language server has an Install button.** The editor named the server, printed a command and left the user to it — a shell, a package manager and a "then reopen this file" for something PPM can do itself. One press installs it and the open file picks the server up in place, with no reopen; the command stays underneath for anyone who would rather run it themselves. The rule it was written against still holds: nothing installs because a file was opened, and pressing a button that says Install is the user asking.
  - **Into `<ppm dir>/lsp-servers`, never `-g`.** A global `typescript@5` replaces whatever TypeScript the user has installed globally, which PPM has no business doing, and a global bin directory is only found when it happens to be on the *service's* PATH — four entries for a systemd-started PPM here. PPM's own directory is neither, `PPM_HOME` already isolates it for tests, and `bun remove` is the whole uninstall. The same arrangement extensions have had since they shipped.
  - Ten servers come from npm and get the button: TypeScript/JavaScript, Pyright, JSON, HTML, CSS, YAML, Bash, Intelephense, Vue and Svelte — each checked to answer `initialize` when started this way (23, 16, 12, 17, 16, 16, 11, 21, 21 and 24 capabilities respectively). clangd, Solargraph and lua-language-server keep a command to copy and no button: they come from a system package manager, i.e. a password prompt and a choice between pacman, apt and brew that PPM has no business making.
  - **Go and Rust have the button too, and neither one is an `npm install` in disguise.** gopls is built by the host's own Go with `GOBIN` pointed at PPM's directory, so `~/go/bin` is left exactly as it was and the module and build caches stay where Go already keeps them (measured: 1.1 s with a warm cache, and the user's own gopls untouched). rust-analyzer is the deliberate exception to installing into PPM's directory at all — it is a rustup *component*, so PPM runs `rustup component add` **in the project's own directory** and keeps no copy: a private one would be the wrong server the moment a repository pins its own toolchain (1.9 s, landing in `~/.rustup/toolchains/<toolchain>/bin`).
  - PPM installs language servers and never toolchains. Where there is no Go or no rustup there is no button at all, only the command — the check is `PATH`, deliberately, because that is the same `PATH` the server would inherit and be unable to run `go list` from.
  - **`~/.cargo/bin/rust-analyzer` lies**: it is a rustup proxy that exists whether or not the component does — measured on a host with none installed, the symlink was there while `rustup which rust-analyzer` said `unknown binary`. Found on `PATH` it would report an installed server that prints that error and exits when spawned, which is a broken server rather than the missing one the button is offered for, so rustup is asked *before* `PATH` and its 3 ms answer is both the lookup and the installed-check. Related trap in the same area: `Bun.which` reads the `PATH` the process **started with** and never looks at `process.env.PATH` again, so a test cannot simulate a missing toolchain by emptying it — only the explicit `{ PATH }` option is live, which is what the lookup now passes.
  - The browser sends a server **id**, and the packages are the registry's — nothing that reaches the route can name a package, a flag or a path. Two presses join one install rather than running two `bun add`s over one lockfile, and the install directory gets its own `package.json` first: measured, `bun add` in a directory with no manifest does not create one there, it walks *up* to the nearest parent that has one and installs into that.
  - **Settings has a Language Servers pane of its own**, because the editor's button only ever appears on a file that already has no server — so setting a language up *before* opening anything in it was impossible, and there was nowhere to see what the machine actually has. It lists all 15 with their languages, which are installed, which are running right now, an Install button on each one PPM can install here, and the command to copy on the ones it cannot. It asks a deliberately machine-wide route (`GET /api/lsp/servers`): no project is open in Settings, so no project's `node_modules/.bin` is consulted and a rustup component lands in the default toolchain — a repository that pins its own is still served by the editor's button, which runs rustup inside that project. Both routers share one install handler, so the id-not-a-package-name property cannot hold on one and not the other. The on/off switch moved here from Appearance, where it was next to word wrap: a server installed while the feature is off does nothing, and the two belong on one pane.
  - **Each of those rows has a Remove, and only the ones PPM installed do.** The gate is not whether the server is installed but *where it was found*, which is the same walk that decides which one gets started: a copy in the project's `node_modules` is the repository's, one on `PATH` is the user's own, and the TypeScript server PPM ships is its own dependency — removing any of the three would be PPM tidying up after somebody else, and the last would break the server a fresh install has. So those rows say `In project`, `On PATH` and `Bundled` where the others say `Installed`, which is also what explains the missing button.
  - **One npm package can provide three servers**, and pressing Remove on JSON removes HTML and CSS with it — they are all `vscode-langservers-extracted`. That is not a thing to find out afterwards, so the confirmation names them: *"HTML and CSS come from the same npm package, so they go too."* Verified in a browser against a real install: one press, three rows back to Install.
...
- **Review a whole branch against another as one list.** A feature branch with sixty commits could only be read one commit at a time, and the alternative — squashing them into one — is not a review. A new tab lists every file that differs between two refs, with the tree on the left, a diff on the right, and a per-file reviewed tick.
  - Every file opens at the **merge base**, not at the base branch's tip. That is what a review means, and the Compare panel had been listing files one way and opening them the other, so the base branch's own commits rendered as deletions inside a branch review.
  - A reviewed tick stores the file's head-side blob id rather than `true`. Keying it to the commit throws all progress away whenever a new one lands; a plain boolean keeps a file ticked after it has been rewritten, which marks unreviewed code as reviewed.
- **Remote Desktop works on Linux hosts.** It was Windows and macOS only. Which backend runs is chosen per *session* rather than per platform, because a Wayland session almost always runs XWayland too: XWayland sets `DISPLAY` and answers an X11 capture **successfully**, with a view of XWayland clients only — a black screen rather than an error. A session is looked up afresh every few seconds, so a desktop that starts after PPM does is found, and an X11 → Wayland re-login stops input being sent to a display that is gone.
  - Keystrokes are sent as evdev codes rather than keysyms. Asking X for "the key that produces an a" answers with the physical Q on an AZERTY host, which is the wrong key on exactly the layouts where it matters.
  - A host missing libX11 is told to install libX11, not libXtst: the capability probe reports each library separately, because `dlopen` resolves all symbols or none, and one absent optional symbol used to cost the host its mouse with a message pointing at the wrong package.
...
### Fixed
- **Tooltips no longer appear under the pointer and stop the panel scrolling.** They opened with no delay, so merely crossing a toolbar fired one — and the box that appeared owned the wheel, because tooltip content is hoverable by default. They now wait half a second, close the moment the trigger is left, and ignore the pointer entirely.
- **The Git Graph is a list of commits again in a panel that is not wide.** With enough parallel branches the graph column grew to whatever the lanes needed and took the whole row: every commit message was zero pixels wide and the columns behind it were clipped off the edge with no way to scroll to them. The graph is now capped to what the panel can spare and pans sideways from a strip in its header, the message column has a floor it cannot go under, and a narrow panel drops date, hash and then changes instead of crushing what is left.
- **Discarding a hunk that had moved could discard the wrong one.** The Source Control panel sends back the position a hunk held when the list was drawn, and a file edited in the meantime moves them. Stage and unstage re-find the hunk by its content, which is right for them — but two identical blocks in one file have the same content, so discarding fell back to the first match and could throw away an edit the user never looked at. Discard now refuses rather than guesses, and says to reload and pick it again. It is the one of the three that cannot be undone.
- **A compacted chat could not be scrolled back into, permanently.** Transcripts over 50MB were refused outright and the route answered 403, which the browser had no way to explain — so a session with thirteen compactions, at 77MB, loaded nothing when you scrolled to the top, every time, for exactly the chats the feature exists for. The cap is 256MB, the message now quotes the size it actually enforces rather than a number from two limits ago, and the reader streams so the file never lands in memory whole.
- **Scrolling back through a compacted chat loads one segment at a time.** It used to answer with everything before the first boundary — 5626 messages at once on that same session, prepended into a view already holding 3553 nodes. Each click now opens the stretch behind the card it was on, and that stretch is headed by the previous boundary's card, so the walk keeps going. On a thread compacted more than once the old answer also skipped the middle stretch entirely, leaving it reachable from nothing at all.
- **Chat search missed most of a long session.** It indexed 179 of one session's 1084 messages, because the walk back through parent links stops at a compaction boundary — correct for the chat view, wrong for a search that is supposed to see the whole history.
...
- **The language server is off until you ask for it, and a phone never starts one.** It is a real process on the host — one `typescript-language-server` on one project was **854 MB** resident — so it now waits to be asked instead of starting because a file was opened. Turn it on from the chip in the editor's breadcrumb bar, from Settings, or from the command palette; turn it off again from the panel that chip opens. The setting is per device, which is the point: a desktop switching it on must not start a server for your phone.
  ...
  - **With the setting off, Monaco's own TypeScript service answers** — completion, hover, signature help — instead of nothing at all. It was unregistered unconditionally, which is right beside a real server (two answers, and the slower one holds the hover widget on "Loading…") and wrong with no server at all: a fresh desktop install and *every* phone had no TypeScript IntelliSense and no setting that could restore it. Its diagnostics stay off, because one file with no `tsconfig.json` and no `node_modules` reports "Cannot find module" for every real import.
  - **A TypeScript server ships with PPM**, so the setting works on a fresh install with nothing else done. A copy in the project's own `node_modules/.bin` still wins, and so does one on `PATH` — the bundled one is the floor, never a preference, or a repository pinned to TypeScript 4 would be analysed by the wrong compiler.
...
- **A first load of 1 MB instead of 33, and an editor that works offline.** Four things were each undoing the others:
  - The service worker was precaching **488 files / 33.3 MB** on first visit — every language grammar, every theme, and a TypeScript worker most sessions never instantiate. It now precaches the shell it actually needs to boot: **9 files / 1.08 MB**, with everything else cached as it is used.
  ...
  - Monaco was loaded from the jsdelivr CDN, so the editor did not work offline, did not work on an air-gapped machine, and told a third party which files you open. It is now staged into the bundle (13.3 MB of the 14.97 MB on disk — the 14 locale bundles are skipped) and served from PPM itself. The 6.7 MB TypeScript worker is 1.2 MB brotli on the wire and is fetched once, on first use, only by a session with no language server running.
